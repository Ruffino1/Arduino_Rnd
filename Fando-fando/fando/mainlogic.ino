void mainlogic() {
  ...
  Hxx  - данные с тензо
  M0/1 - данные от индукционных датчиков
  */
  /* все команды:
  0  - ожидание
  1  - инициализация
  ...
  */
  ...
  switch (step) {
    case 0:          // ожидание
      if (!alarm) {  // не в ошибке
        //led_ind(strip.Color(50, 150, 0, 0));
        //m_select(0); // мультиплексор в 0
      }
      // если на паузе
      ...
      // если в простое сработал датчик руки
      if (!alarm && input_b) {
        if (digitalRead(in_p)) {
          t_old = t_now;
          input_b = false;
      ...
      // если перегруз по тензо
      if (!alarm && t_now > 150) step = 16;
      // каждые пять секунд опрашиваем тензо
      if (last_time < millis() + 5000) {
        tenzo_f_read();        // читаем датчики
        last_time = millis();  // сбрасываем таймер
      }
      break;

    case 1:  // инициализация
      led_all(strip.Color(0, 0, 0, 100));
      led_ind(strip.Color(0, 0, 0, 0));  // выключаем индикатор
      speed = 10;
      h_move(-185570);
      if (ready) {
        delay(700);
        ...
        b1 = true;
        b2 = true;
        tenzo_read(1);  // сбрасываем 0 весов
        step = 12;
      }
      break;
    ...
    case 5:  // Принудительно сбрасываем ошибку
      alarm = false;
      Serial.println("R");
      led_all(strip.Color(0, 0, 0, 100));  // подсветка в дежурный режим
      led_ind(strip.Color(0, 0, 0, 0));    // выключаем индикатор
      step = 0;
      break;
